...
import { Split, Link as LinkIcon, HardDrive } from 'lucide-react';
...
const TextUtilities: React.FC<Props> = ({ metadata, subTool }) => {
  ...
  const process = (val: string, sep: string) => {
    ...
    if (subTool === 'separator') {
        ...
        // Split by typical delimiters and join with new one
        const parts = val.split(/[\s,\.\-]+/).filter(Boolean);
        setOutput(parts.join(char));
    } else if (subTool === 'extractor') {
        const urls = val.match(/(https?:\/\/[^\s]+)/g);
        setOutput(urls ? urls.join('\n') : 'No URLs found.');
    } else if (subTool === 'size') {
      ...
      <div className="p-6 space-y-6">
        {subTool === 'separator' && (
             <div className="flex items-center gap-4 mb-4">
                <label className="text-sm font-medium text-slate-700">Separate By:</label>
                <select 
                    value={separator} 
                    onChange={(e) => { setSeparator(e.target.value); process(input, e.target.value); }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {subTool === 'size' ? (
                 <div className="bg-slate-50 border border-slate-200 rounded-lg p-6 flex flex-col justify-center items-center space-y-4">
                    <HardDrive className="h-12 w-12 text-brand-600 mb-2" />
                    <div className="text-center">
                        <div className="text-3xl font-bold text-slate-900">{sizeInfo.bytes} <span className="text-lg font-normal text-slate-500">Bytes</span></div>
                        <div className="text-xl font-semibold text-slate-700">{sizeInfo.kb} <span className="text-sm font-normal text-slate-500">KB</span></div>
                         <div className="text-lg text-slate-600">{sizeInfo.mb} <span className="text-sm font-normal text-slate-500">MB</span></div>
                    </div>
                 </div>
            ) : (
                <div>
