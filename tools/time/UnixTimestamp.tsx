...
const UnixTimestamp: React.FC<{ metadata: ToolMetadata }> = ({ metadata }) => {
  ...
  return (
    <ToolTemplate
      metadata={metadata}
      howItWorks="Convert Unix timestamps (seconds since Jan 1 1970) to human readable dates and vice-versa."
    >
      <div className="p-6 space-y-8">

        {/* CURRENT TIMESTAMP */}
        <div className="bg-brand-50 p-6 rounded-xl border border-brand-100 text-center">
          <span className="text-sm font-bold text-brand-600 uppercase tracking-wider mb-2 block">
            Current Unix Timestamp
          </span>
          <div className="text-4xl font-mono font-bold text-brand-900">
            {current}
          </div>
        </div>

        {/* 🔥 AD – AFTER CURRENT TIMESTAMP */}
        <AdNative />
