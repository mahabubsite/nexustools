...
const YoutubeThumbnail: React.FC<{ metadata: ToolMetadata }> = ({ metadata }) => {
  ...
  return (
    <ToolTemplate 
      metadata={metadata} 
      howItWorks="Paste a YouTube video URL to instantly view and download thumbnails in multiple resolutions."
    >
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          <div className="relative">
            ...
            <input 
              ...
              className="block w-full pl-10 pr-4 py-4 border border-slate-300 rounded-xl focus:ring-2 focus:ring-brand-500 text-lg"
            />
          </div>
...
const ThumbnailCard: React.FC<{ title: string; size: string; url: string; primary?: boolean }> = ({ title, size, url, primary }) => {
  ...
  return (
    <div className={`bg-white rounded-xl border ${primary ? 'border-brand-200 shadow-md' : 'border-slate-200 shadow-sm'} overflow-hidden`}>
      <div className="aspect-video bg-slate-100 relative group">
        ...
        <div className="absolute inset-0 bg-black/10 opacity-0 group-hover:opacity-100 flex items-center justify-center transition">
          <button 
            onClick={download}
            className="bg-white px-4 py-2 rounded-full font-bold flex items-center gap-2 shadow"
          >
